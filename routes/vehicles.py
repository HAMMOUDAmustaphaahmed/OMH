...
# Create the upload folder structure
UPLOAD_FOLDER = Path(Config.UPLOAD_FOLDER)
VEHICLES_UPLOAD_FOLDER = UPLOAD_FOLDER / 'vehicles'
VEHICLES_UPLOAD_FOLDER.mkdir(parents=True, exist_ok=True)
        ...
        if 'image' in request.files:
            ...
            if file and file.filename != '' and allowed_file(file.filename):
                try:
                    ...
                    # Ensure upload directory exists and construct full path
                    upload_path = VEHICLES_UPLOAD_FOLDER / unique_filename
                    ...
                    relative_path = upload_path.relative_to(UPLOAD_FOLDER)
                    new_vehicle.image_url = f'/static/uploads/{relative_path}'
                    
                except Exception as e:
                    flash(f'Erreur lors de l\'upload de l\'image: {str(e)}', 'danger')
                    return redirect(url_for('vehicles.add'))
